...
const GOOGLE_SHEET_URL = "https://script.google.com/macros/s/AKfycbxPZ64tDbJQ7pWRkuDzt8JnBd9YpK5XNs-aM1DMOwQlQTwfa-eUBqVkyclTfs7kCyNokA/exec";
...
// ============================================================
// TASK LIST — Structured exercises for participants
// ============================================================
const TASKS = {
  en: [
...
const T = {
  en: {
    ...
    disclosureItems: [
      ["Purpose", "This study is conducted as part of a Bachelor thesis research project investigating human interaction with AI-based avatars. Your participation helps us understand how people communicate with AI systems."],
      ["How it works", "You will have a 90-second conversation with AURA, an AI avatar. You can speak using your microphone or type your messages. The avatar will respond in real time."],
      ...
      ["Data & Privacy", "The conversation data collected is used solely for academic research purposes. No personal data will be shared with third parties. Participation is voluntary."],
      ["Microphone access", "This app requires microphone access for voice input. You may also type if you prefer not to use the microphone."],
    ],
    consentText: "I have read and understood the above information. I consent to participate in this study and agree that my conversation data may be used for academic research purposes.",
    ...
    chatTitle: "Chatting with AURA",
    live: "Live", offline: "Offline",
    pressEnter: "Press Enter · Or use mic",
    tasksTitle: "Your Tasks",
    tasksSub: "Try to complete each task during your 5 minutes",
    sessionComplete: "Session Complete",
    avatarGreeting: "Hello! I'm AURA, your AI avatar. You have 90 seconds to chat with me. Feel free to speak or type — I'm listening!",
  ...
  de: {
    ...
    disclosureItems: [
      ["Zweck", "Diese Studie ist Teil einer Bachelorarbeit, die die Interaktion zwischen Menschen und KI-basierten Avataren untersucht. Ihre Teilnahme hilft uns zu verstehen, wie Menschen mit KI-Systemen kommunizieren."],
      ["Ablauf", "Sie haben ein 90-sekündiges Gespräch mit AURA, einem KI-Avatar. Sie können per Mikrofon sprechen oder Nachrichten eintippen. Der Avatar antwortet in Echtzeit."],
      ["Sitzungsdauer", "Ihre Sitzung endet automatisch nach 90 Sekunden. Danach werden Sie gebeten, einen kurzen Fragebogen auszufüllen."],
      ["Daten & Datenschutz", "Die Daten werden ausschließlich für akademische Forschungszwecke verwendet. Keine persönlichen Daten werden an Dritte weitergegeben. Die Teilnahme ist freiwillig."],
      ["Mikrofonzugang", "Diese App benötigt Mikrofonzugang für die Spracheingabe. Sie können auch tippen."],
    ],
    consentText: "Ich habe die obigen Informationen gelesen und verstanden. Ich erkläre mich damit einverstanden, an dieser Studie teilzunehmen.",
    ...
    chatTitle: "Gespräch mit AURA",
    live: "Live", offline: "Offline",
    pressEnter: "Enter zum Senden · Oder Mikrofon",
    tasksTitle: "Ihre Aufgaben",
    tasksSub: "Versuchen Sie, jede Aufgabe in 90 Sekunden zu erledigen",
    sessionComplete: "Sitzung abgeschlossen",
    avatarGreeting: "Hallo! Ich bin AURA, Ihr KI-Avatar. Sie haben 90 Sekunden Zeit. Sprechen oder tippen Sie — ich höre zu!",
...
async function saveToDatabase(payload: {
  ...
  try {
    ...
    console.error("[AURA] Database save failed:", err);
    return false;
  }
}
...
// ============================================================
// PAGE 4 — Questionnaire + Supabase save
// ============================================================
function QuestionnairePage({ lang, userInfo, messageCount }: { lang: Lang; userInfo: UserInfo; messageCount: number }) {
  ...
  const handleSubmit = async () => {
    ...
    // Build flat arrays for Supabase
    ...
    return (
      <main className="min-h-screen bg-void flex items-center justify-center p-6">
        <div className="max-w-lg w-full text-center space-y-8 animate-fade-in">
          ...
          <div className="space-y-3">
            <p className="font-mono text-xs text-sage uppercase tracking-widest">{t.sessionComplete}</p>
              ...
              {section.questions.map((q, qi) => (
                <div key={qi} className={`p-4 space-y-3 transition-colors ${!ratings[q] && missingCount > 0 ? "bg-pulse/5" : ""}`}>
                  ...
                  <div className="flex items-center gap-2">
                    {[1,2,3,4,5].map((val) => (
                      <button key={val} onClick={() => { setRatings((prev) => ({ ...prev, [q]: val })); if (missingCount > 0) setMissingCount((c) => c - 1); }}
...
// ============================================================
// PAGE 3 — Avatar Session with Task Panel
// ============================================================
export default function HomeClient() {
  ...
  const startTimer = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => { if (prev <= 1) { stopSession(true); return 0; } return prev - 1; });
    }, 1000);
  }, [stopSession]);
  ...
  const processUserMessage = useCallback(async (userText: string) => {
    ...
    setStatus("thinking");
    try {
      // 1. Call OpenAI for a real AI response
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: userText.trim(), conversationHistory: messages.slice(-10).map(m => ({ role: m.role, content: m.content })) }),
      });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.error || "API error");
      const reply = data.reply;

      // 2. Show reply in chat
      addMessage("assistant", reply);

      // 3. Speak via avatar if connected, otherwise use browser TTS
      if (avatarRef.current) {
        try {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      // Bind video when stream is ready
      avatar.on(SessionEvent.SESSION_STREAM_READY, () => {
        console.log("[LiveAvatar] Stream ready — binding video");
        if (videoRef.current) {
          avatar.attach(videoRef.current);
          videoRef.current.play().catch(console.warn);
        }
      });
      // AVATAR_TRANSCRIPTION disabled — messages added directly by processUserMessage
      ...
      // Attach video and start session
      avatar.attach(videoRef.current);
      await avatar.start();
  ...
  const timerColor = timeLeft > 120 ? "#4ECDC4" : timeLeft > 60 ? "#FBBF24" : "#FF6B6B";
  ...
  return (
    <main className="min-h-screen bg-void flex overflow-hidden" style={{ height: "100vh" }}>
      ...
      <div className="relative flex-1 bg-obsidian overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-6 py-4">
          ...
          {isSessionActive && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl"
              style={{ background: "rgba(5,5,8,0.7)", border: `1px solid ${timerColor}40`, animation: timeLeft <= 60 ? "breathe-glow 1s ease-in-out infinite" : "none" }}>
        ...
        {/* Task panel — floating bottom-left */}
        {isSessionActive && (
          <div className="absolute bottom-24 left-4 z-30 w-72">
            <div className="glass-card rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(0,212,255,0.2)" }}>
              {/* Task header — clickable to collapse */}
              <button onClick={() => setShowTasks(!showTasks)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-ghost font-mono text-[10px]">
              ...
              {showTasks && (
                <div className="p-3 space-y-2">
                  ...
                  {tasks.map((task) => {
                    ...
                    return (
                      <button key={task.id} onClick={() => setCompletedTasks((prev) => done ? prev.filter(id => id !== task.id) : [...prev, task.id])}
                        ...
                        <p className={`font-body text-xs leading-relaxed transition-colors ${done ? "text-silver/40 line-through" : "text-silver/80"}`}>
                          {task.text}
                        </p>
                      </button>
                    );
